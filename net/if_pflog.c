...
inline struct pflog_softc	*pflog_getif(int);

struct if_clone	pflog_cloner =
    IF_CLONE_INITIALIZER("pflog", pflog_clone_create, pflog_clone_destroy);

inline int
pflog_softc_cmp(const struct pflog_softc *a, const struct pflog_softc *b)
{
	return (a->sc_unit - b->sc_unit);
}

RBT_HEAD(pflog_tree, pflog_softc) pflog_ifs = RBT_INITIALIZER(&pflog_ifs);

RBT_PROTOTYPE(pflog_tree, pflog_softc, sc_entry, pflog_softc_cmp);
RBT_GENERATE(pflog_tree, pflog_softc, sc_entry, pflog_softc_cmp);
...
pflog_clone_create(struct if_clone *ifc, int unit)
{
	...
	NET_LOCK();
	RBT_INSERT(pflog_tree, &pflog_ifs, pflogif);
	NET_UNLOCK();
...
pflog_clone_destroy(struct ifnet *ifp)
{
	...
	NET_LOCK();
	RBT_REMOVE(pflog_tree, &pflog_ifs, pflogif);
	NET_UNLOCK();
...
inline struct pflog_softc *
pflog_getif(int unit)
{
	return RBT_FIND(pflog_tree, &pflog_ifs, (struct pflog_softc *)&unit);
}
