...
unsigned int			pflog_ifs_num = 0;
SMR_LIST_HEAD(, pflog_softc)	pflog_ifs =
    SMR_LIST_HEAD_INITIALIZER(pflog_ifs);
...
pflog_clone_create(struct if_clone *ifc, int unit)
{
	...
	if (pflog_ifs_num >= PFLOG_IFS_MAX)
		return (ENOMEM);
	++pflog_ifs_num;
	...
	NET_LOCK();
	SMR_LIST_INSERT_HEAD_LOCKED(&pflog_ifs, pflogif, sc_entry);
	NET_UNLOCK();
...
pflog_clone_destroy(struct ifnet *ifp)
{
	...
	NET_LOCK();
	SMR_LIST_REMOVE_LOCKED(pflogif, sc_entry);
	NET_UNLOCK();
...
pflog_getif(int unit)
{
	...
	SMR_LIST_FOREACH_LOCKED(pflogif, &pflog_ifs, sc_entry) {
