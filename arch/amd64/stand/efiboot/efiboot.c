...
efi_cleanup(void)
{
	...
	UINTN			 mem_map_size, map_key, desc_size;
	UINT32			 desc_version;

	/* retry once in case of failure */
	for (retry = 5; retry >= 0; retry--) {
		mem_map_size = 0;
		status = EFI_CALL(BS->GetMemoryMap, &mem_map_size, NULL,
		...
		status = EFI_CALL(BS->GetMemoryMap, &mem_map_size, mem_map,
		    &map_key, &desc_size, &desc_version);
		if (status != EFI_SUCCESS)
			panic("cannot get the memory map (%u)", status);
		status = EFI_CALL(BS->ExitBootServices, IH, map_key);
		if (status == EFI_SUCCESS)
			break;
		if (retry == 0) {
for (volatile long long i = 0; i < 10000000000LL; i++) continue;
			panic("ExitBootServices failed (%u)", status);
		}
	}
}
