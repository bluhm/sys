    ...
    bus_space_handle_t h2, bus_size_t o2, size_t c)
{
	...
	if (addr1 >= addr2) {
		...
		for (bus_size_t i = c; i > 0; --i)
			ghcb_io_write_1(addr2 + i - 1, ghcb_io_read_1(addr1 + i - 1));
	}
}
    ...
    bus_space_handle_t h2, bus_size_t o2, size_t c)
{
	...
	if (addr1 >= addr2) {
		for (bus_size_t i = 0; i < c; ++i)
			ghcb_io_write_2(addr2 + 2*i, ghcb_io_read_2(addr1 + 2*i));
	} else {
		for (bus_size_t i = c; i > 0; --i)
			ghcb_io_write_2(addr2 + 2*(i-1), ghcb_io_read_2(addr1 + 2*(i-1)));
	}
}
    ...
    bus_space_handle_t h2, bus_size_t o2, size_t c)
{
	...
	if (addr1 >= addr2) {
		for (bus_size_t i = 0; i < c; ++i)
			ghcb_io_write_4(addr2 + 4*i, ghcb_io_read_4(addr1 + 4*i));
	} else {
		for (bus_size_t i = c; i > 0; --i)
			ghcb_io_write_4(addr2 + 4*(i-1), ghcb_io_read_4(addr1 + 4*(i-1)));
	}
}
    ...
    bus_space_handle_t h2, bus_size_t o2, size_t c)
{
	...
	if (addr1 >= addr2) {
		for (bus_size_t i = 0; i < c; ++i)
			ghcb_mem_write_1(addr2 + i, ghcb_mem_read_1(addr1 + i));
	} else {
		for (bus_size_t i = c; i > 0; --i)
			ghcb_mem_write_1(addr2 + i - 1, ghcb_mem_read_1(addr1 + i - 1));
	}
}
    ...
    bus_space_handle_t h2, bus_size_t o2, size_t c)
{
	...
	if (addr1 >= addr2) {
		for (bus_size_t i = 0; i < c; ++i)
			ghcb_mem_write_2(addr2 + 2*i, ghcb_mem_read_2(addr1 + 2*i));
	} else {
		for (bus_size_t i = c; i > 0; --i)
			ghcb_mem_write_2(addr2 + 2*(i-1), ghcb_mem_read_2(addr1 + 2*(i-1)));
	}
}
    ...
    bus_space_handle_t h2, bus_size_t o2, size_t c)
{
	...
	if (addr1 >= addr2) {
		for (bus_size_t i = 0; i < c; ++i)
			ghcb_mem_write_4(addr2 + 4*i, ghcb_mem_read_4(addr1 + 4*i));
	} else {
		for (bus_size_t i = c; i > 0; --i)
			ghcb_mem_write_4(addr2 + 4*(i-1), ghcb_mem_read_4(addr1 + 4*(i-1)));
	}
}
    ...
    bus_space_handle_t h2, bus_size_t o2, size_t c)
{
	...
	if (addr1 >= addr2) {
		for (bus_size_t i = 0; i < c; ++i)
			ghcb_mem_write_8(addr2 + 8*i, ghcb_mem_read_8(addr1 + 8*i));
	} else {
		for (bus_size_t i = c; i > 0; --i)
			ghcb_mem_write_8(addr2 + 8*(i-1), ghcb_mem_read_8(addr1 + 8*(i-1)));
	}
}
